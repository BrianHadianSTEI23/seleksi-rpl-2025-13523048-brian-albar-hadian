...
export default function SubsidyDetail() {
  ...
  // Update helpers (same as before)
  const updatePacket = (index: number, key: "name" | "value", value: string) => {
    const newPackets = [...subsidyData.packets];
    newPackets[index][key] = key === "value" ? Number(value) : value;
    setSubsidyData({ ...subsidyData, packets: newPackets });
  };

  const updateSource = (index: number, key: "name" | "value", value: string) => {
    const newSources = [...subsidyData.sources];
    newSources[index][key] = key === "value" ? Number(value) : value;
    setSubsidyData({ ...subsidyData, sources: newSources });
  };

  // --- New saveChanges function ---
  const saveChanges = () => {
    if (!category) return;

    // Overwrite the data in the original subsidyDetailData
    subsidyDetailData[category as keyof typeof subsidyDetailData] = {
      ...subsidyData,
      packets: [...subsidyData.packets],
      sources: [...subsidyData.sources],
      vendor: { ...subsidyData.vendor }
    };

    console.log("Updated subsidyDetailData:", subsidyDetailData);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
                ...
                <CardDescription>Breakdown realisasi bantuan berdasarkan paket program</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                {subsidyData.packets.map((packet: any, idx: number) => (
                    <div key={packet.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                        <Badge variant="outline" className="font-mono">{packet.id}</Badge>
                        <input
                            type="text"
                            value={packet.name}
                            onChange={(e) => updatePacket(idx, "name", e.target.value)}
                            className="font-semibold text-gray-900 border-b border-gray-300 focus:outline-none"
                        />
                        </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        className="font-bold text-green-600 text-lg w-40 text-right border-b border-gray-300 focus:outline-none"
                        />
                    </div>
                    </div>
                ))}
            ...
            <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
                ...
                <CardDescription>Sumber-sumber pendanaan untuk program bantuan sosial</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                {subsidyData.sources.map((source: any, idx: number) => (
                    ...
                    <div className="flex items-center">
                        ...
                        className="font-medium text-gray-900 border-b border-gray-300 focus:outline-none"
                        />
                    </div>
                    <input
                        type="number"
                        ...
                        className="font-bold text-blue-600 text-right w-40 border-b border-gray-300 focus:outline-none"
                    />
                    </div>
                ))}
            ...
            <div className="space-y-6">
              ...
              <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
                  ...
                  <div>
                      <input
                      type="text"
                      value={subsidyData.vendor.name}
                      onChange={(e) => setSubsidyData({ ...subsidyData, vendor: { ...subsidyData.vendor, name: e.target.value } })}
                      className="font-semibold text-gray-900 w-full border-b border-gray-300 focus:outline-none"
                      />
              ...
              <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
                  ...
                  <div className="space-y-3">
                      ...
                      <input
                          type="number"
                          value={subsidyData.target}
                          onChange={(e) => setSubsidyData({ ...subsidyData, target: Number(e.target.value) })}
                          className="font-bold text-purple-600 text-right w-40 border-b border-gray-300 focus:outline-none"
                      />
                      </div>
                      ...
                      <input
                          ...
                          onChange={(e) => setSubsidyData({ ...subsidyData, deadline: e.target.value })}
                          className="font-semibold text-gray-900 mt-1 w-full border-b border-gray-300 focus:outline-none"
                      />
                      </div>
                  </div>
                  </CardContent>
              ...
              <Card className ="bg-green-600 hover:bg-green-700 text-white" onClick={saveChanges}>
                  <CardHeader>
                    <CardTitle className="flex justify-center items-center text-white text-xl font-semibold" >
                        Simpan perubahan
                    </CardTitle>
                  </CardHeader>
              </Card>
